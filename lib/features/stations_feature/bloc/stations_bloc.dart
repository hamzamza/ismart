...
class StationsBloc extends Bloc<StationsEvent, StationsState> {
  ...
  StationsBloc(this.stationsRepository) : super(const StationsState()) {
    ...
    on<ChangePageDuration>(_onchangeduration);
    on<ChangeingPageEvent>(_onchangepage);
  ...
  Future<void> _onchangeduration(
    ChangePageDuration event,
    Emitter<StationsState> emit,
  ) async {
    emit(state.copyWith(ischarging: event.charging));
  }
